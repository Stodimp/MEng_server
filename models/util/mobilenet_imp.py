...
from keras.layers import BatchNormalization, ReLU, GlobalAveragePooling2D, Reshape, Activation
...
def mobilenet_block_swish(tensor_in, filters: int, strides: int, block_id: int):
    x = DepthwiseConv2D(kernel_size=3, strides=strides,
                        padding='same', name="conv_dw_%d" % block_id,)(tensor_in)
    x = BatchNormalization(name="conv_dw_%d_bn" % block_id)(x)
    x = Activation("swish", name="conv_dw_%d_swish" % block_id)(x)
    x = Conv2D(filters=filters, kernel_size=1, strides=1,
               name="conv_pw_%d" % block_id,)(x)
    x = BatchNormalization(name="conv_pw_%d_bn" % block_id)(x)
    x = Activation("swish", name="conv_pw_%d_swish" % block_id)(x)
    return x
...
def build_mobilenet_swish(output_nodes: int, model_name: str = "mobilenet") -> Model:
    # Start of model:
    inputs = Input(shape=(None, None, None, 2), name="input_layer")
    ...
    # Middle part
    x = mobilenet_block_swish(x, filters=64, strides=1, block_id=1)
    x = mobilenet_block_swish(x, filters=128, strides=2, block_id=2)
    x = mobilenet_block_swish(x, filters=128, strides=1, block_id=3)
    x = mobilenet_block_swish(x, filters=256, strides=2, block_id=4)
    x = mobilenet_block_swish(x, filters=256, strides=1, block_id=5)
    x = mobilenet_block_swish(x, filters=512, strides=2, block_id=6)
    for i in range(5):
        x = mobilenet_block_swish(x, filters=512, strides=1, block_id=i+7)
    x = mobilenet_block_swish(x, filters=1024, strides=2, block_id=12)
    x = mobilenet_block_swish(x, filters=1024, strides=1, block_id=13)
    # output
    x = GlobalAveragePooling2D(name='global_avg_pooling')(x)
    outputs = Dense(units=output_nodes, activation='sigmoid',
                    name='predictions')(x)
